...
There are 9127 rows and 275 columns. The types of the variables are numeric, factor, logical and integer.

##  Use `ggplot2` to draw a scatterplot of height (`HEIGHT3`) and weight (`WEIGHT2`), facet by gender (`SEX1`). State your expectation regarding the relationship between the variables, comment on the plot you see.
```{r}
iowa %>% ggplot(aes(x = HEIGHT3, y = WEIGHT2)) + geom_point() + facet_grid(~SEX1)
```

There is maybe a posistive linear relationship between height and weight in all genders. But there are still lots of the points which do not follow the relastionship. So it it hard to recognize the relationshiop visually.

## Temporarily restrict weight and height to below 2500, then plot the values again. Describe the plot you see.
```{r}
iowa %>% filter(HEIGHT3< 2500 & WEIGHT2<2500 ) %>% ggplot(aes(x= HEIGHT3, y = WEIGHT2)) + geom_point()+ facet_grid(~SEX1)
```

From this plot, We can not recognize the evident relationship between weight and height. Most of point are located at the area of 0 to 400 in weight and 400 to 600 in height.

## Use a combination of `filter` and logical expressions to answer the following questions:
#a. What proportion of the height measurements are taken in metric measurements? 
```{r}
metric <- iowa %>% filter(HEIGHT3>=9000 & HEIGHT3<=9998)
nrow(metric)/nrow(iowa)
```
...
```{r}
iowa %>% filter(HEIGHT3>=200 & HEIGHT3<=711, SEX1 == 1 | SEX1 == 2) %>% group_by(SEX1) %>% summarize(min(HEIGHT3),max(HEIGHT3))
iowa %>% filter(HEIGHT3>=200 & HEIGHT3<=711, SEX1 == 1 | SEX1 == 2) %>% ggplot(aes(x = HEIGHT3)) + geom_histogram(binwidth = 100) + facet_wrap(~SEX1)
```

From the histogram, we can see that the heigh of most female are located in 450 to 550. More than half of the male have height in 450 to 550 while some male have height in 550 to 650. We can also see that the average hight of male will be higher than female. I choose 100 as the binwidth so that the overall height level can be showed in the histogram.
...
```{r}
iowa %>% group_by(NUMADULT) %>% summarise(n=n()) %>% arrange(desc(n)) 
```

From the table, we can see that the mode is 2.
...
```{r}
iowa %>% group_by(EDUCA) %>% summarise(n=n()) %>% mutate (percent = n/sum(n))
```

From the table, we can see that the percentage of graduates in Iowa is 32.67% which is smaller than the nation's average 37.42%.

# c. Out of the people asked, what percentage of people got their flu shot (`FLSHTMY2`)  in July 2017 or after?
```{r}
a <- iowa %>% filter(FLSHTMY2 < 777777,  (FLSHTMY2 %/% 10000 >= 7 & FLSHTMY2 %% 10000 == 2017) | (FLSHTMY2 %% 10000 == 2018)) 
nrow(a)/nrow(iowa %>% filter(FLSHTMY2 <= 999999))
```
